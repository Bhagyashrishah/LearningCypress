describe('Signetic Test Suite', () => {
    ...
    beforeEach(() => {
        ...
        cy.intercept('GET', '/api/v2/validateAddress').as('addressValidation');
     ...
     //   cy.wait('@addressValidation');
        ...
        // Intercepting appointment submission request
        cy.intercept('POST', '/api/v2/appointment').as('submitAppointment');

        // Submitting the appointment
        cy.get('[data-qa="submit-appointment"]').click();
        cy.wait('@submitAppointment').then((interception) => {
            // Extracting appointment details from the request
            appointmentDetails = interception.request.body;
        });
    ...
    it('should successfully reschedule the appointment', () => {
        ...
        cy.get('.text-demibold').contains('09:30 am').click();
